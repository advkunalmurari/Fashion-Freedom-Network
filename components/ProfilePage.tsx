...
export const ProfilePage: React.FC<{ user: User; onBack: () => void }> = ({ user, onBack }) => {
  ...
  const [activeView, setActiveView] = useState<'portfolio' | 'saved'>('portfolio');
        ...
        <section className="flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="flex-1 space-y-12 pt-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-8 border-y border-gray-100">
              ...
              <div className="space-y-1"><p className="text-[9px] uppercase tracking-widest text-gray-400 font-black">Network</p><p className="text-2xl font-serif italic text-ffn-black">{user.followersCount.toLocaleString()}+</p></div>
              <div className="space-y-1"><p className="text-[9px] uppercase tracking-widest text-gray-400 font-black">Rank Hub</p><p className="text-2xl font-serif italic text-ffn-primary">TOP {user.rankPosition || '10'}</p></div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <button
                ...
                disabled={isActionLoading}
                className="bg-gray-50 text-ffn-black flex-1 py-7 rounded-[2rem] text-[10px] font-bold uppercase tracking-[0.4em] flex items-center justify-center space-x-3 group hover:bg-white hover:shadow-xl transition-all border border-gray-100"
              >
                <MessageSquarePlus className="w-5 h-5 text-ffn-primary" /> <span>Collab Inquiry</span>
              </button>
              <button onClick={() => setShowModelCard(true)} className="bg-white border border-gray-100 text-ffn-black flex-1 py-7 rounded-[2rem] text-[10px] font-bold uppercase tracking-[0.4em] flex items-center justify-center space-x-3 hover:bg-gray-50 transition-all shadow-sm">
                <FileText className="w-5 h-5 text-ffn-primary" /> <span>Model Composite</span>
              </button>
            </div>
          </div>
        </section>
        ...
        {showHireModal && (
          <div className="fixed inset-0 z-[1100] flex items-center justify-center p-6 bg-ffn-black/60 backdrop-blur-md">
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white w-full max-xl rounded-[3.5rem] p-12 md:p-16 space-y-12 shadow-2xl">
              <div className="flex items-center justify-between">
                <div className="space-y-1"><h3 className="text-3xl font-serif italic text-ffn-black">Dispatch Inquiry</h3><p className="text-[9px] uppercase tracking-[0.3em] font-black text-gray-400">AUTHORITY DISCOVERY PROTOCOL</p></div>
                <button onClick={() => setShowHireModal(false)} className="p-4 bg-gray-50 rounded-2xl"><X className="w-6 h-6" /></button>
              </div>
              <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); setShowHireModal(false); alert("Protocol Inquiry Dispatched."); }}>
                <div className="space-y-2"><label className="text-[10px] uppercase tracking-widest font-black text-gray-400">Company Name</label><input required className="w-full bg-gray-50 border-none rounded-2xl p-5 text-sm" placeholder="e.g. Milan Studios" /></div>
                <div className="space-y-2"><label className="text-[10px] uppercase tracking-widest font-black text-gray-400">Message</label><textarea required className="w-full bg-gray-50 border-none rounded-2xl p-5 text-sm h-32 resize-none" placeholder="Describe the collaboration..." /></div>
                <button type="submit" className="w-full bg-ffn-black text-white py-6 rounded-[2rem] text-[10px] font-bold uppercase tracking-[0.4em] flex items-center justify-center space-x-3 shadow-2xl hover:bg-ffn-primary transition-all">
                  <Send className="w-5 h-5" /> <span>Activate Narrative</span>
                </button>
              </form>
            </motion.div>
          </div>
