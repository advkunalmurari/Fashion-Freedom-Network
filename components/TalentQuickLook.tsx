...
export const TalentQuickLook: React.FC<TalentQuickLookProps> = ({ talent, onClose, onViewFullProfile }) => {
    return (
        <motion.div
            ...
            className="fixed top-0 right-0 w-full md:w-[450px] h-full bg-white z-[200] shadow-[-20px_0_60px_rgba(0,0,0,0.1)] overflow-hidden"
        >
            {/* Header / Cover */}
            <div className="h-64 relative">
                <img
                    src={talent.coverUrl || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop'}
            ...
            <div className="px-10 -mt-20 relative z-10 pb-12 overflow-y-auto h-[calc(100%-16rem)] no-scrollbar">
                <div className="space-y-8">
                    ...
                    <div className="flex items-end space-x-6">
                        <div className="w-32 h-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden bg-gray-100">
                            <img src={talent.avatarUrl} className="w-full h-full object-cover" alt={talent.displayName} />
                        </div>
                        <div className="pb-2">
                            <div className="flex items-center space-x-2">
                                <h3 className="text-3xl font-serif italic text-ffn-black">{talent.displayName}</h3>
                                {talent.isVerified && <CheckCircle className="w-5 h-5 text-ffn-primary fill-ffn-primary/10" />}
                            </div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-1">{talent.role} • {talent.location}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100">
                            ...
                            <p className="text-2xl font-serif italic text-ffn-black">{talent.stats?.reliability || 98}%</p>
                        </div>
                        <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100">
                            ...
                            <p className="text-2xl font-serif italic text-ffn-black">Top 1%</p>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full py-6 bg-ffn-primary text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl shadow-ffn-primary/30 hover:bg-ffn-black transition-all flex items-center justify-center space-x-4"
                    >
