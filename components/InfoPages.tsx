  ...
  <div className="space-y-32 py-20 animate-in fade-in duration-1000 bg-ffn-black text-white">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
      {[
        ...
        <div key={i} className="bg-white/5 p-12 rounded-[3.5rem] border border-white/10 shadow-xl space-y-8 group hover:shadow-ffn-primary/10 transition-all backdrop-blur-3xl">
          <div className="w-16 h-16 bg-ffn-primary/10 rounded-2xl flex items-center justify-center text-ffn-primary group-hover:bg-ffn-primary group-hover:text-white transition-all"><box.icon className="w-8 h-8" /></div>
          <h3 className="text-3xl font-serif italic text-white">{box.title}</h3>
          <p className="text-sm text-white/50 leading-relaxed font-light">{box.text}</p>
        </div>
      ))}
    </div>
...
export const FAQPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-20 space-y-20 animate-in fade-in duration-700 bg-ffn-black text-white">
      <header className="text-center space-y-6">
        <HelpCircle className="w-16 h-16 text-ffn-primary mx-auto opacity-20" />
        <h1 className="text-6xl font-serif italic text-white tracking-tighter">Help & FAQ</h1>
        <p className="text-[10px] uppercase tracking-[0.5em] font-black text-white/20">FFN Knowledge Protocol</p>
      </header>
      <div className="space-y-8">
        {faqs.map((faq, i) => (
          <div key={i} className="bg-white/5 p-10 rounded-[2.5rem] border border-white/10 shadow-lg space-y-4 group backdrop-blur-3xl">
            <h3 className="text-xl font-serif italic text-white group-hover:text-ffn-primary transition-colors">{faq.q}</h3>
...
export const CommunityGuidelines: React.FC = () => (
  <div className="max-w-4xl mx-auto py-20 space-y-16 animate-in fade-in duration-700 bg-ffn-black text-white">
    <header className="text-center space-y-6">
      <ShieldAlert className="w-16 h-16 text-ffn-accent mx-auto opacity-20" />
      <h1 className="text-6xl font-serif italic text-white tracking-tighter">Community Protocol</h1>
      <p className="text-[10px] uppercase tracking-[0.5em] font-black text-white/20">Rules of Professional Conduct</p>
    </header>
    <div className="bg-white/5 rounded-[4rem] border border-white/10 p-12 md:p-20 shadow-xl space-y-12 backdrop-blur-3xl">
      <section className="space-y-4">
        <h3 className="text-2xl font-serif italic text-white">Identity Integrity</h3>
        <p className="text-sm text-white/60 leading-relaxed">Impersonation of agencies, brands, or other talent is strictly prohibited. All professional hub details must be accurate and owned by the account holder.</p>
      </section>
      <section className="space-y-4">
        <h3 className="text-2xl font-serif italic text-white">Professional Discourse</h3>
        <p className="text-sm text-white/60 leading-relaxed">The network is for professional discovery. Harassment, non-consensual solicitation, or unprofessional conduct in direct messaging will result in immediate protocol suspension.</p>
      </section>
      <section className="space-y-4">
        <h3 className="text-2xl font-serif italic text-white">Copyright Sovereignty</h3>
        <p className="text-sm text-white/60 leading-relaxed">Ensure you have the rights to all media uploaded. If showcasing work from a collaboration, proper attribution to photographers, stylists, and MUAs is required.</p>
      </section>
    </div>
  </div>
);
  ...
  return (
    <div className="py-20 space-y-24 animate-in fade-in duration-1000 bg-ffn-black text-white">
      <header className="max-w-4xl mx-auto text-center space-y-8">
        ...
        <h1 className="text-7xl font-serif italic text-white tracking-tighter leading-none">Invest in Your <br /> Identity.</h1>
        <p className="text-xl text-white/40 font-light italic">Lifecycle fees and elite membership tiers for global professional sovereignty.</p>
      </header>

      {/* One-time Fees */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-6xl mx-auto">
        {/* Identity Activation */}
        <div className="bg-white/5 p-16 rounded-[4rem] border border-white/10 shadow-2xl space-y-10 relative overflow-hidden group backdrop-blur-3xl">
          ...
          <div className="space-y-4">
            <h3 className="text-4xl font-serif italic text-white leading-none">Professional <br /> Listing.</h3>
            <p className="text-6xl font-serif font-bold tracking-tighter text-white">{PRICING.CURRENCY}{PRICING.PROFILE_LISTING}</p>
          </div>
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <h2 className="text-4xl font-serif italic text-center text-white">Membership Protocols</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {/* Professional Subscription */}
          <div className="bg-white/5 p-12 rounded-[3rem] border border-white/10 shadow-xl space-y-8 backdrop-blur-3xl">
            <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-ffn-primary">Protocol: Professional</h4>
            <div className="space-y-2">
              <p className="text-5xl font-serif font-bold text-white">{PRICING.CURRENCY}{PRICING.SUBSCRIPTION.PROFESSIONAL}<span className="text-sm font-light text-white/40">/mo</span></p>
            </div>
...
export const VerificationPage: React.FC = () => (
  <div className="max-w-4xl mx-auto py-20 space-y-20 animate-in fade-in duration-700 bg-ffn-black text-white">
    <header className="text-center space-y-8">
      ...
      <h1 className="text-7xl font-serif italic text-white tracking-tighter leading-none">Identity <br /> Verification.</h1>
      <p className="text-xl text-white/40 font-light italic">The FFN badge is more than an icon; it is a seal of professional excellence.</p>
    </header>

    <div className="space-y-12">
      <div className="bg-white/5 p-12 md:p-20 rounded-[4rem] border border-white/10 shadow-2xl space-y-12 backdrop-blur-3xl">
        <div className="space-y-6">
          <h3 className="text-3xl font-serif italic text-white">How it Works</h3>
          <p className="text-sm text-white/50 leading-relaxed font-light">Verification involves a multi-layered check of your industry activity. We analyze your digital footprint across Instagram, editorial publications, and professional collaborations to ensure you are a master of your craft.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-4">
            <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">Benefits</h4>
            <ul className="space-y-4">
              {["High-Level Scout Visibility", "Trusted Partner Status", "Access to Pro-Only Channels", "Increased Inquiry Conversion"].map((b, i) => (
                <li key={i} className="flex items-center space-x-3 text-[10px] text-white/40 font-bold uppercase tracking-widest"><CheckCircle className="w-4 h-4 text-emerald-500" /> {b}</li>
              ))}
            </ul>
          </div>
          <div className="space-y-4">
            <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">Requirements</h4>
            <ul className="space-y-4">
              {["Minimum 3 Portfolio Items", "Linked Social Identity", "Verified Professional Category", "One-Time Activation Fee"].map((r, i) => (
                <li key={i} className="flex items-center space-x-3 text-[10px] text-white/40 font-bold uppercase tracking-widest"><ArrowRight className="w-4 h-4 text-ffn-primary" /> {r}</li>
              ))}
            </ul>
          </div>
        </div>
