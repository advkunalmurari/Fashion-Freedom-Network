            ...
            <form onSubmit={nextStep} className="space-y-10 animate-in slide-in-from-right-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  <select title="Category Selection" required className="w-full py-6 px-8 text-sm focus:border-ffn-primary transition-all bg-white appearance-none">
          ...
          {step === 3 && (
            <div className="space-y-12 animate-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-6">
                <p className="text-[8px] uppercase tracking-[0.4em] text-gray-400 text-center font-bold">Secured by PayPal Encryption</p>
                {isProcessing ? (
                  <button disabled className="w-full bg-ffn-primary text-white py-8 rounded-[2.5rem] text-[12px] font-bold uppercase tracking-[0.5em] flex items-center justify-center space-x-6 hover:bg-ffn-primary/90 transition-all shadow-2xl shadow-ffn-primary/40">
                    <Loader2 className="w-6 h-6 animate-spin" />
                    <span>Activating Profile...</span>
                  </button>
                ) : (
                  <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-xl">
                    <p className="text-[10px] uppercase tracking-[0.4em] font-bold text-ffn-primary mb-6 text-center">Secure Payment Gateway</p>
                    <PayPalButton
                      amount={PRICE_VAL.PROFILE_LISTING.toString()}
                      currency="USD"
                      onSuccess={handlePaymentSuccess}
                    />
                    <button
                      onClick={() => handlePaymentSuccess({ id: 'offline_bypass' })}
                      className="w-full mt-4 py-3 text-[9px] uppercase font-bold text-gray-400 hover:text-ffn-black transition-colors"
                    >
                      Bypass for Testing
                    </button>
                  </div>
